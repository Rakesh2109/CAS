...
Figure~\ref{fig:binary-sweep} follows the two sensitivity slices in
Fig.~\ref{fig:sweep}. At $B=15$, varying $\pi_{\mathrm{thr}}$ gives
macro-F1 0.820--0.822 for full CAS and 0.754--0.755 for pruned CAS.
At $\pi_{\mathrm{thr}}=0.8$, varying $B$ from 5 to 25 gives
0.820--0.821 and 0.754--0.756, respectively. Both pools are relatively
insensitive within these slices; increasing $B$ does not close the
performance gap caused by pruning.

\begin{figure}[!htb]
\color{blue}
\centering
\includegraphics[width=\columnwidth]{fig_binary_sweep_full_pruned_column.pdf}
...
CAS converts TM clauses into signed, inspectable evidence. Multiclass
pruning retains 390 of 2,400 candidates and improves selected macro-F1
from 0.765 to 0.787, with reduced threshold sensitivity in
Fig.~\ref{fig:sweep}. Binary pruning retains only 30 of 800 candidates
but lowers CAS macro-F1 from 0.823 to 0.756. Figure~\ref{fig:binary-sweep}
shows that this gap persists across the tested sensitivity slices.
Compactness therefore has a task-dependent predictive cost.

Full-pool binary CAS improves precision but raises the missed-attack
fraction from 1.3\% to 9.0\%. Operating-point selection must reflect
false-alarm and missed-attack costs. The pruning margin condition and
CPSS bound address prediction preservation and low-selection-probability
inclusions, respectively; neither certifies causal explanations.
...
and runtime and energy measurements remain necessary to assess
