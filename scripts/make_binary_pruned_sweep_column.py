"""Binary counterpart of Figure 2, using audited three-way results."""
import json
...
audit = json.loads((FIGS.parent / "paper/review/binary_revision/results_audit.json").read_text())
data = {"baseline": {k: v["mean"] for k, v in audit["pools"]["pruned"]["tm"].items()}}
experiment = json.loads((FIGS.parent / "results/threeway_binary_pruned.json").read_text())
plt.rcParams.update({"font.size": 8, "axes.grid": True, "grid.alpha": .3,
                     "lines.linewidth": 1.3, "errorbar.capsize": 2,
                     "pdf.fonttype": 42, "svg.fonttype": "none", "text.color": "blue",
                     "axes.labelcolor": "blue", "axes.titlecolor": "blue",
                     "xtick.color": "blue", "ytick.color": "blue"})
...
for extension in ("pdf", "svg", "png"):
    fig.savefig(FIGS / f"fig_binary_sweep_full_pruned_column.{extension}", dpi=220)
plt.close(fig)
