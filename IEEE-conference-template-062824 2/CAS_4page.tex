...
\begin{IEEEkeywords}
attack attribution, intrusion detection, interpretable ML,
...
to select compact, reusable class signatures from a trained clause pool. More specifically, a weight-based pruning first reduces the candidate pool, and it then applies complementary-pairs stability selection (CPSS)~\cite{meinshausen2010,shah2013} to the retained clauses' activations. Further, averaged coefficients from
...
%\begin{itemize}
%\item A clause-based attribution layer that combines CPSS with signed, normalized evidence to construct reusable class signatures.
%\item A sufficient margin condition for prediction-preserving TM pruning and an explicit account of the scope of CPSS error control.
%\end{itemize}
The main contributions are: (1) a CPSS-based attribution layer with signed, normalized evidence for reusable class signatures; and (2) a sufficient margin condition for prediction-preserving TM pruning and a characterization of CPSS error control.


\begin{figure*}[!t]
\centering
\includegraphics[width=0.8\textwidth]{fig_cpss_protocol.pdf}
...
For $N_{\mathrm{cls}}$ classes and $C$ clauses per class, the trained pool produces Boolean activations
...
$Z\in\{0,1\}^{n\times p}$, initially with $p=N_{\mathrm{cls}}C$.
...
\label{sec:cpss}
For each class $\ell$, CPSS divides the fit partition into $B$ complementary pairs as shown in Fig~1. On each of the $2B$ halves, a class-balanced
...
\begin{equation}
 ...
 \label{eq:sign}
\end{equation}
Positive coefficients provide inculpatory evidence while negative ones provide exculpatory evidence. The attribution rule is
\begin{equation}
...
quantiles. The TM hyperparameter used is $C=400$ clauses per class (200 positive and
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\columnwidth]{fig_signature_frontier.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\columnwidth]{fig_sweep_heldout.pdf}
...
To assess pruning alone, Fig.~\ref{fig:pruning} shows
the performance across removal budgets. Report-set macro-F1 and
...
(Table~\ref{tab:headline}). Pruning thus removes most of the
pool with a mean macro-F1 decrease of $0.0045$.

\begin{figure}[t]
\centering
\includegraphics[width=0.8\columnwidth]{fig_clause_pruning_heldout.pdf}
